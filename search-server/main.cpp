//Итоговы1 проект. Спринт 1. Рудиков А.В.
...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, Func func) const {            
        ...
        sort(matched_documents.begin(), matched_documents.end(),
             [](const Document& lhs, const Document& rhs) {
                if (abs(lhs.relevance - rhs.relevance) < project_error) {
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status) const {            
        return FindTopDocuments(raw_query, [&status](int document_id, DocumentStatus in_status, int rating){
