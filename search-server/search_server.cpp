...
void SearchServer::AddDocument(int document_id, std::string_view document, DocumentStatus status,
	...
	for (std::string_view word : words) {
		...
		doc_to_words_freqs_[document_id][word] = word_to_document_freqs_[word][document_id];
	}
...
std::vector<Document> SearchServer::FindTopDocuments(std::string_view raw_query) const {
	return FindTopDocuments(std::execution::seq,
		raw_query, DocumentStatus::ACTUAL);
}
...
SearchServer::MatchDocumentResult SearchServer::MatchDocument(const std::execution::parallel_policy&,
	...
	if (documents_.count(document_id) == 0) {
		using namespace std::literals::string_literals;
		throw std::out_of_range("Incorrect document_id"s);
	}
	bool skip_sort = true;
	const auto query = ParseQuery(raw_query, skip_sort);
	const auto status = documents_.at(document_id).status;
...
void SearchServer::RemoveDocument(ExecutionPolicy&& policy,
	int document_id) {
	const auto& word_freqs = doc_to_words_freqs_.at(document_id);
...
SearchServer::Query SearchServer::ParseQuery(std::string_view text, bool skip_sort = false) const {
	Query result;
